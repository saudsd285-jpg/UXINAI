...
import { User, Rocket, Star, Hash } from "lucide-react";
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data, error } = await supabase.auth.signInAnonymously({
        options: {
          data: { full_name: username.trim(), display_name: username.trim() },
        },
      });
      ...
      if (code && FRIEND_CODES[code]) {
        const friend = FRIEND_CODES[code];
        setTimeout(() => {
          toast({
            title: `🌟 مرحباً ${friend.name}!`,
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <div className="relative">
              ...
              <input
                ...
                required
                className="w-full pr-10 pl-4 py-3.5 rounded-xl bg-[hsl(230,20%,14%)]/80 border border-[hsl(217,91%,60%)]/20 text-white placeholder:text-[hsl(215,20%,45%)] text-sm outline-none focus:ring-2 focus:ring-[hsl(217,91%,60%)]/40 focus:border-[hsl(217,91%,60%)]/50 transition-all"
              />
            </div>
